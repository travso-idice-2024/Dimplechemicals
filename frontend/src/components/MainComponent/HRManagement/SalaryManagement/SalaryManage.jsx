...
const SalaryManage = () => {
  ...
  const [users, setUsers] = useState(() => {
    const storedUsers = localStorage.getItem("SalaryManage");
    return storedUsers
      ? JSON.parse(storedUsers)
      : [
          {
            baseslary: "25000",
            bonusincentives: "10% per project",
            duduction: "Tax",
            overtime: "2500",
            Finalsalary: "300000",
            slaryMonth: "25000",
            BankName: "Axis",
            Account: "832695456202",
            IFSC: "UI45678",
            Paymenet: "Cash"
          },
          {
  ...
  useEffect(() => {
    localStorage.setItem("SalaryManage", JSON.stringify(users));
  }, [users]);
  ...
  const filteredUsers = users
    .sort((a, b) => b.baseslary.localeCompare(a.baseslary))
    .filter(
      (user) =>
        user.baseslary.toLowerCase().includes(searchTerm.toLowerCase())
    );
